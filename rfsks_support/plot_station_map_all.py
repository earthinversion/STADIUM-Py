...
def plot_station_map_all(retr_stationsfile = "results/InfoRF/all_stations_rf_retrieved.txt",all_stationsfile = "results/InfoRF/all_stations_RF.txt"):
    ...
    if not os.path.exists(figname):
        ...
        all_nets_set = set(all_stations_df['#Network'].values)
        ...
        for net,stalon,stalat in zip(all_stations_df['#Network'].values,all_stations_df['Longitude'].values,all_stations_df['Latitude'].values):
            
            x,y = map(stalon, stalat)
            # if avail:
            #     map.plot(x, y,'^', color=netcolor[net], markersize=7,markeredgecolor=netcolor[net],linewidth=0.1)
            # else:
            map.plot(x, y,'^', markerfacecolor="None", markersize=7,markeredgecolor='k', markeredgewidth=0.5,zorder=2)

        for net,stalon,stalat in zip(retr_stations_df['#Network'].values,retr_stations_df['Longitude'].values,retr_stations_df['Latitude'].values):
            
            x,y = map(stalon, stalat)
            # if avail:
            map.plot(x, y,'^', color=netcolor[net], markersize=7,markeredgecolor=netcolor[net],linewidth=0.1,zorder=3, markeredgewidth=0.5)
            # else:
            # map.plot(x, y,'^', markerfacecolor="None", markersize=7,markeredgecolor='k', markeredgewidth=5)
